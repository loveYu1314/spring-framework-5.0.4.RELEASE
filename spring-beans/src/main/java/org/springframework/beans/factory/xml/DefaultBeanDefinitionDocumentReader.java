...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		//具体的解析过程委派给 BeanDefinitionParserDelegate 实现
		// BeanDefinitionParserDelegate 中定义了Spring Bean定义XML文件的各种元素
	...
	protected BeanDefinitionParserDelegate createDelegate(
		...
		// BeanDefinitionParserDelegate 初始化Document根元素
	 ...
	 */
	//使用Spring的Bean规则从Document的根元素开始解析Bean定义的Document
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					if (delegate.isDefaultNamespace(ele)) {
						...
						//如果没有Spring默认的XML命名空间，则使用用户自定义的解析规则解析元素节点
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		//如果导入元素的location属性值为空，说明没有导入任何元素，直接返回
	...
	protected void processAliasRegistration(Element ele) {
		...
		//<Alias>别名元素中alias的属性值
		String alias = ele.getAttribute(ALIAS_ATTRIBUTE);
		boolean valid = true;
		//<Alias>别名元素中name属性值为空
		if (!StringUtils.hasText(name)) {
			getReaderContext().error("Name must not be empty", ele);
			valid = false;
		}
		//<Alias>别名元素中alias属性值为空
		...
		if (valid) {
			...
			//再解析完<Alias>别名元素之后，发送容器别名处理完成事件
